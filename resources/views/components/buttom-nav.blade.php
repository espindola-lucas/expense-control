<nav class="fixed bottom-0 left-0 w-full bg-white border-t border-gray-300 flex justify-around items-center h-14 shadow-inner z-50">
    ...
    <form method="POST" action="{{ route('logout') }}" class="flex flex-col items-center text-sm text-back hover:bg-blue-700 hover:text-white hover:py-2 hover:px-4 hover:rounded-lg hover:text-white hover:bg-blue-700">
        @csrf
